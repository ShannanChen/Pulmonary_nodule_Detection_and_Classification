...
def plotnoduledist(annopath):
    import pandas as pd 
    df = pd.read_csv(annopath+'train/y-annotations.csv')
    diameter = df['diameter_mm'].reshape((-1,1))

    df = pd.read_csv(annopath+'val/y-annotations.csv')
    diameter = np.vstack([df['diameter_mm'].reshape((-1,1)), diameter])

    df = pd.read_csv(annopath+'test/y-annotations.csv')
    diameter = np.vstack([df['diameter_mm'].reshape((-1,1)), diameter])
    fig = plt.figure()
...
def getrecallwrtdetp(path='/media/data1/wentao/tianchi/CTnoddetector/training/detector/results/res18/baselinebboxlranchorftbig96/test64/'):
    ...
    annogt = pd.read_csv('/media/data1/wentao/tianchi/csv/newtest/y-annotations.csv', names=gtcolname)
...
if __name__ == '__main__':
    ...
    # calrecall('/mnt/media/wentao/tianchi/csv/val/y-annotations.csv')
    # calrecall('/mnt/media/wentao/tianchi/csv/test/y-annotations.csv')
