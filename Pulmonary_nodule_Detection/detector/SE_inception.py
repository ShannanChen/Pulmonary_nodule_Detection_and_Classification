...
from layerincep import Inception_v1, Inception_v2, BasicConv, Deconv, SingleConv,se_res_conc_block,SELayer3D
...
class Net(Module):
    def __init__(self):
        ...
        self.se1=SELayer3D(32,16)
        self.se2 = SELayer3D(64, 16)
        self.conv2 = se_res_conc_block(32, 32)
        self.downsample2 = Inception_v1(32, 64)
        self.conv3 = se_res_conc_block(64, 64)
        self.downsample3 = Inception_v1(64,64)#64, 128)
        self.conv4 = se_res_conc_block(64,64)#128, 128)
        self.downsample4 = Inception_v1(64,64)#64,64)#64,128)#128, 128)

        self.conv4_ = SingleConv(64, 64)  # 128, 128)
        self.incept4 = se_res_conc_block(64, 64)  # 64,64)#128, 128)
        self.deconv4 = Deconv(64, 64)  # 64,64)#128, 128)

        self.conv5 = SingleConv(128, 64)  # 128,64)#192, 128)
        self.incept5 = se_res_conc_block(64, 64)  # 64,64)#128, 128)
    ...
    def forward(self, x, coord):
        ...
        se1 = self.se1(down1)
        conv2 = self.conv2(se1)  # (32,32,32)

        down2 = self.downsample2(conv2)  # (16,16,16)
        se2 = self.se2(down2)
        conv3 = self.conv3(se2)  # (16,16,16)

        down3 = self.downsample3(conv3)  # (8,8,8)
        se3 = self.se2(down3)
        conv4 = self.conv4(se3)  # (8,8,8)

        down4 = self.downsample4(conv4)  # (4,4,4)
        se4 = self.se2(down4)
        conv4_ = self.incept4(self.conv4_(se4))
