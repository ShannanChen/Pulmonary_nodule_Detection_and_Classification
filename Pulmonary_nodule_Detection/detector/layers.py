...
def hard_mining(neg_output, neg_labels, num_hard):
    _, idcs = torch.topk(neg_output, min(num_hard, len(neg_output)))   #返回neg_output的从大到小的数值和索引的前min(num_hard, len(neg_output))多个
...
class Loss(nn.Module):
    def __init__(self, num_hard = 0):
        ...
        self.classify_loss = nn.BCELoss() #二分类用的交叉熵
    ...
    def forward(self, output, labels, train = True):
        ...
        if len(pos_output)>0:
            print 'pos_prob num: ',len(pos_output)
...
def nms(output, nms_th):
    ...
    output = output[np.argsort(-output[:, 0])]    # np.argsort() 将-output[:, 0]按数字大小输出所对应的索引
    bboxes = [output[0]]
