...
# #####20180405 将每个样本预测出来的pbb和lbb，整合为一个pbb和lbb
# save_dir = '/media/cad/c5290228-7b88-43ba-809c-870ee6577d2c/data/zyc/chengxu/DeepLung-master/detector/results/res18-20180404-214820/bbox/'#'results/ma_offset40_res_n6_100-1/'
# pbbfile=glob(os.path.join(save_dir, '*pbb.npy'))
# pbb=[]
# lbb=[]
# for pbb_npy in pbbfile:
#     p=np.load(pbb_npy)
#     l=np.load(pbb_npy.replace("pbb", "lbb"))
#     pbb.append(p)
#     lbb.append(l)
...
def get_pr(pbb, lbb, conf_th, nms_th, detect_th, pid, tp_list, fp_list, p_list):
    ...
    for i in range(len(pbb)):
        ...
        tpi, fpi, pi,lll = acc(pbb[i], lbb[i], conf_th, nms_th, detect_th)
        tp += tpi
        fp += fpi
        p += pi
    tp_list[pid] = tp
