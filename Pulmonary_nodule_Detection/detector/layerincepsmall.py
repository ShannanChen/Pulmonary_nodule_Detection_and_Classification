...
class Inception_v1(nn.Module):
    def __init__(self, cin, co, relu=True, norm=True):
        ...
        self.branch3 = nn.Sequential(OrderedDict([
            ...
            ('relu1', nn.ReLU(inplace=True)),
            ('conv2', nn.Conv3d(2 * cos[2], cos[2], 3, stride=2, padding=1)),
            ('norm2', nn.BatchNorm3d(2 * cos[2])),
            ('relu2', nn.ReLU(inplace=True)),
            ('conv3', nn.Conv3d(2 * cos[2], cos[2], 3, stride=2, padding=1)),
...
class Inception_v2(nn.Module):
    def __init__(self, cin, co, relu=True, norm=True):
        ...
        self.branch3 = nn.Sequential(OrderedDict([
            ...
            ('norm1', nn.BatchNorm3d(2 * cos[2])),
            ('relu1', nn.ReLU(inplace=True)),
            ('conv2', nn.Conv3d(2 * cos[2], cos[2], 3, stride=1, padding=1)),
            ('norm1', nn.BatchNorm3d(2 * cos[2])),
            ('relu1', nn.ReLU(inplace=True)),
            ('conv3', nn.Conv3d(2 * cos[2], cos[2], 3, stride=1, padding=1)),
        ]))
