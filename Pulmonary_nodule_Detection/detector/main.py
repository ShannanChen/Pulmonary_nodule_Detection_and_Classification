...
parser.add_argument('--model', '-m', metavar='MODEL', default='like9_opti_0713',
                    help='model')
...
parser.add_argument('--start-epoch', default=130, type=int, metavar='N',
                    help='manual epoch number (useful on restarts)')#0
parser.add_argument('-b', '--batch-size', default=16, type=int,
                    metavar='N', help='mini-batch size (default: 16)')  #16
parser.add_argument('--lr', '--learning-rate', default=0.001, type=float,
                    metavar='LR', help='initial learning rate') #default 0.01 incep-res     #0.001 res
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        #train(train_loader, net, loss, epoch, optimizer, get_lr, args.save_freq, save_dir)   #yuan

        metricstrain = train(train_loader, net, loss, epoch, optimizer, get_lr,
                             args.save_freq, save_dir)    #####zyc
        validate(val_loader, net, loss,metricstrain,epoch)
...
def train(data_loader, net, loss, epoch, optimizer, get_lr, save_freq, save_dir):
    ...
    print
    return metrics    ####zyc
#def validate(data_loader, net, loss):#yuan
def validate(data_loader, net, loss,metricstrain, epoch):    #####zyc
    start_time = time.time()
    ...
    print
    print
    #tpr真阳率 （灵敏度)       tnr  真阴性(特异度)


    from loggeradd import Logger1

    logger = Logger1('./logs')
    # (1) Log the scalar values
    info = {
        'losstrain': np.mean(metricstrain[:, 1]),
        'lossval': np.mean(metrics[:, 1])
    }

    for tag1, value1 in info.items():
        logger.scalar_summary(tag1, value1, epoch)

    info1 = {

        'l-tprtrain': 100.0 * np.sum(metricstrain[:, 6]) / np.sum(metricstrain[:, 7]),
        'l-tprval': 100.0 * np.sum(metrics[:, 6]) / np.sum(metrics[:, 7])
    }
    for tag1, value1 in info1.items():
        logger.scalar_summary(tag1, value1, epoch)

    info2 = {

        'l-tnrtrain': 100.0 * np.sum(metricstrain[:, 8]) / np.sum(metricstrain[:, 9]),
        'l-tnrval': 100.0 * np.sum(metrics[:, 8]) / np.sum(metrics[:, 9])
    }
    ...
    info3 = {
        ...
        'classify loss': np.mean(metricstrain[:, 1]),
	'classify tnrval': np.mean(metrics[:, 1])
    }
    ...
    info4 = {
        ...
        'regress d train': np.mean(metricstrain[:, 2]),
        'regress d test': np.mean(metrics[:, 2])
    }
    for tag, value in info4.items():
        logger.scalar_summary(tag, value, epoch)
    info5 = {

        'regress x train': np.mean(metricstrain[:, 3]),
        'regress x test': np.mean(metrics[:, 3])
    }
    for tag, value in info5.items():
        logger.scalar_summary(tag, value, epoch)
    info6 = {

        'regress y train': np.mean(metricstrain[:, 4]),
        'regress y test': np.mean(metrics[:, 4])
    }
    for tag, value in info6.items():
        logger.scalar_summary(tag, value, epoch)

    info7 = {

        'regress z train': np.mean(metricstrain[:, 5]),
        'regress z test': np.mean(metrics[:, 5])
    }
    for tag, value in info7.items():
        logger.scalar_summary(tag, value, epoch)

    info8 = {
        'traintotal neg': np.sum(metricstrain[:, 9]),
        'valtotal neg': np.sum(metrics[:, 9])
    }

    for tag1, value1 in info8.items():
        logger.scalar_summary(tag1, value1, epoch)
    ...
    # for i, (data, target, coord) in enumerate(data_loader):
    #     info = {
    #         'images': to_np(data.view(-1, 96, 96,96)[:5])
    #     }
    #
